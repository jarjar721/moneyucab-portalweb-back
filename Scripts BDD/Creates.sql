...
(
  PE_PK SERIAL,
  PE_DESCRIPCION VARCHAR(240) NOT NULL,
  PRIMARY KEY (PE_PK)
);
...
(
  US_PK SERIAL,
  US_TIPO VARCHAR(1) NOT NULL,
  US_CORREO VARCHAR(60) NOT NULL,
  US_USERNAME VARCHAR(100) NOT NULL,
  US_CLAVE VARCHAR(500) NOT NULL,
  PRIMARY KEY (US_PK)
);

CREATE TABLE Clave_Historia
(
  CH_PK SERIAL,
  CH_CLAVE VARCHAR(50) NOT NULL,
  CH_FECHA_USO DATE NOT NULL,
  PRIMARY KEY (CH_PK)
);

CREATE TABLE Usuario_Clave_Historia
(
  US_CH_PK SERIAL,
  US_FK INTEGER NOT NULL,
  CH_FK INTEGER NOT NULL,
  US_COUNT INTEGER NOT NULL,
  PRIMARY KEY (US_CH_PK),
  CONSTRAINT USUARIO_CLAVE_HISTORIA_USUARIO_ID FOREIGN KEY (US_FK)
      REFERENCES  Usuario(US_PK) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT USUARIO_CLAVE_HISTORIA_CLAVE_HISTORIA_ID FOREIGN KEY (CH_FK)
      REFERENCES Clave_Historia(CH_PK) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION
);
...
(
  RO_PK SERIAL,
  US_FK INTEGER NOT NULL,
  PE_PK INTEGER NOT NULL,
  PRIMARY KEY (RO_PK),
  CONSTRAINT ROL_USUARIO_ID FOREIGN KEY (US_FK)
      REFERENCES Usuario(US_PK) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT ROL_PERMISO_ID FOREIGN KEY (PE_PK)
      REFERENCES Permiso(PE_PK) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION	
);
